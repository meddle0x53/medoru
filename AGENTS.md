...
**Status**: v0.8.0 in progress. Link previews feature fixed and verified: fetcher no longer crashes on successful responses, LiveViews subscribe to pending previews so first-time URLs render after fetch, and Earmark no longer corrupts autolinked URLs or drops trailing text. Admin word kanji de-association, learned kanji practice pagination, grammar text-section truncation fix, draft lesson preview crash fix, grammar word color fixes, and duplicate white-board route cleanup are complete. English-learning UI wiring complete. Daily Radical Hunt challenge complete and mobile-friendly. AI Word Enrichment remaining. Mature word content filtering complete. Gender dropdown in profile settings no longer resets when toggling checkboxes. Push notifications for classroom chats now open `/classrooms/<id>?tab=chat`. Kanji radical data bug fixed: 沢 now maps to 水 instead of 火. Admin user impersonation ("Login as") added. Admin user list shows last login timestamp. Presentation mode bug fixes: fullscreen background fills width on first entry, content scrolls to prevent kanji breakdown cut-off, vocabulary audio updates per slide. White board/stream posts and comments support copy-paste image uploads. Command parsers strip spaces around expressions. Grammar lesson from image now preserves examples in the description for both grammar and text steps and slices lesson title/description to fit validation limits. Vocabulary lesson from image now falls back to reading when AI omits text, strips optional brackets from expressions while preserving the bracketed form in notes, and prompts the AI to include katakana words, expressions, and phrases. Copy-to-wordset button and word-count badge removed from grammar lessons in classroom lesson lists. Grammar index page now highlights learned grammar points with a green border and "Learned" badge for logged-in users. Admin kanji form supports AI enrichment for metadata, readings, and stroke data, with local KanjiVG fallback for strokes. Classroom vocabulary test generation fixed: timer no longer uses `max_attempts` as seconds, `reading_text` and `image_to_meaning` question types render and score correctly, `kanji_writing` steps use the required 5 points, and teachers can set optional due dates and max attempts from the generation UI. Teachers can now also choose whether multichoice distractors are drawn from the selected words only or from all words in the classroom. A new classroom kanji drawing test generator lets teachers create tests from kanji found in vocabulary lessons; each kanji is worth its stroke-count in points, with one point subtracted per wrong stroke, and points are added to the student's classroom total only on completion.  
**Tests**: 1567 passing  
...
- **Map progression / repeatable battle fix**: Battle tiles are now marked `completed` immediately on victory in `BattleScene.onBattleEnd` (before `WinScene`), so gambling rewards can no longer leave the tile active. The hero marker stays on the completed tile and its outgoing edges become the highlighted choices; completed tiles no longer show the active pulse and display a green checkmark.
...
- **WinScene gamble kanji variety**: `WinChallengeSystem.pickKanji()` now picks a random learned kanji with stroke data instead of always selecting the lowest-stroke one.
- **Replace-ability dialog overflow fix**: The "Ability Cap Reached" dialog in `WinScene` is now scrollable, so a full ability list no longer overflows the panel. Wheel scrolling works, and rows outside the visible area are hidden and disabled.
- **Map progression skip fix**: `Player.completeTile()` is now idempotent and leaves `currentTileId` on the completed tile. This fixes the double-skip caused by `BattleScene` and `WinScene` both calling it for the same battle tile, and keeps the highlighted edges attached to the finished node instead of jumping ahead. `MapScene.onTileClick()` no longer ignores the current tile when it is non-battle, so shop/rest/memory tiles can actually be entered.
...
- **Cascade input clearing**: The input field is now cleared immediately when a word is answered correctly, so the player can start typing the next word without manually deleting.
- **Cascade word pool fix**: `Admin.GameLive` and `GameApiController` now merge both `list_learned_words/2` and `list_english_learned_words/2`, then de-duplicate by word ID. The client `CascadeScene.buildWordList()` was filtering on `w.text`, but the server sends `word`; updated to accept either key. This mismatch was the actual reason all learned words were discarded and only the 3 fallback words appeared.
- **Cascade keyboard layout**: The space key was being drawn with a huge height because `createKey` used the same value for width and height; it now accepts separate width/height. Letter rows are raised slightly and the control row has more vertical gap, so C/V/B/N are no longer overlapped by the space bar.
- **Cascade skip**: Pressing the `ArrowDown` key manually crashes the current word (costs one life) and prevents the default browser scrolling behavior.
- **Cascade crashed-word answer**: When a word is lost—either by reaching the danger line or by skipping—the correct answer is shown above the danger line for ~3.5s before fading.
- **Cascade timer start fix**: `CascadeScene` no longer starts the 60-second timer in `create()`. The game begins in a paused "Get Ready" state and only starts the timer after the player clicks, presses a key, or after a 2-second auto-start, preventing "Time is up" from firing before the cascade is visible.
- **Cascade repeat-word fix**: Spawning now advances a dedicated `wordIndex` instead of using `wordsDestroyed`, so a word that is skipped or crashed is not immediately followed by the same word.
- **JSON-driven map system**: Map definitions moved from hardcoded JS to `assets/js/game/data/maps/`. Added `japanese_fields.json` (Level 1) and `dummy.json` (Level 2) with per-column tile weights, background/tile image placement (offset + scale), and per-column enemy pools. `MapGenerator.js`, `BootScene.js`, `MapScene.js`, `Player.js`, and `enemies/index.js` updated to consume the registry. Existing Level 1 visuals and enemy selection are preserved; `MAP_VERSION` was bumped to reset old saves.
- **Chest tile**: Added `ChestScene` routed from `TILE_TYPES.CHEST`. Rewards are generated by `ChestRewards.js`: 20-80 gold, 1-3 rarity-rolled items, 90% chance for an ability or weapon charm (50/50 split) with 70/15/10/5 rarity weights, 10% chance for a weapon/shield upgrade (player choice when both are under +10), and 5% chance for +1 stat point. Ability rarities in `warrior.json` normalized from `normal` to `common`. Added closed/open chest scene backgrounds and a crossfade opening animation.
- **Meta-progression refactor**: Added `HeroSelectScene` as the entry point and `HomeShopScene` for the home tile. Run-scoped state (gold, inventory, equipped charms/abilities, stat allocations, map) is now reset each run while persistent meta state (`gameTokens`, `rareGameTokens`, `unlocked*` pools, `permanentWeaponLevel`, `permanentShieldLevel`, `permanentStatPointBonus`) is preserved. `Player.endRun()` awards +1 game token on boss victory, a 5% rare token chance, and unlocks the next locked item from `metaUnlocks.json`. Boss defeat and victory both return to `HeroSelectScene`. `LoadoutScene` supports a pre-map mode launched from the home camp. Abilities are filtered by `unlockedAbilityIds`. `MapScene` has DEV `RESET RUN` and `HARD RESET` buttons that return to hero select.
- **Run start cleanup fix**: `Player.resetToFreshHero()` now starts the player with empty `ownedCharmIds`, `ownedSocketCharmIds`, `activeItemIds`, and `inventory` so hard resets no longer give free charms/items. `LoadoutScene` only shows owned charms/items, and `BattleScene` only shows owned active items.
- **Hero stat/base stat tuning**: Brand-new/hard-reset heroes now start with 0 allocatable stat points (points must be bought permanently from the home camp). Warrior base stats changed to `capacity: 5` and `strength: 15`.
- **Map progress persistence fix**: `HeroSelectScene` now resumes an existing run (`MapScene`) if a saved map is present, and only resets to a fresh hero when there is no saved map. Page refreshes no longer wipe current tile progress.
- **Boot flow**: `BootScene` now skips the hero-select screen entirely when an active run is saved, so refreshing mid-run drops you straight back onto the map.
- **Final-map win screen**: Added `RunVictoryScene` shown after defeating a boss on a map marked `"isFinal": true` (set on `japanese_fields.json`). It displays the +1 game token, any super game token (5% chance), and the next unlocked ability, then returns to hero select. Non-final bosses continue to the next map as before. Added `"isFinal": false` to `dummy.json`; the final check now defaults to `true` when the flag is missing, so the level 1 map always triggers the victory screen even if a stale build lacks the flag.
- **Site-level token catch-up**: Added `savedSiteLevel` to the loadout. On a fresh save it equals your site level. Each time a new run starts (`resetToFreshHero`), the saved level is compared to your current site level; for every level gained in between, `N` game tokens are added to your saved tokens and `savedSiteLevel` is updated. No site queries happen during a run.
- **New enemy: Hitotsume-kozō**: Added `hitotsume_kozo.json` with three abilities:
  - `Eye Bonk` — cheap physical attack.
  - `Frightening Gaze` — gaze attack that applies `weak` (50%) and `slow` (20%).
  - `Evil Eye` — void-element attack that applies `void_touched` (20%) and `weak` (10%).
  - Both gaze abilities have a 50% word-chance challenge: success halves the status chances, failure boosts them by 110%.
- New statuses: `slow` (next turn stamina ×0.75) and `void_touched` (incoming damage +25%).
- Enemy challenge engine extended to support `halveChance` / `boostChance` outcomes and pass `effectChanceMultiplier` through `Enemy.performAction` to `applyAbilityEffects`.
- Hitotsume-kozō added to the Japanese Fields battle pools (columns 1–7) and registered in `ENEMY_DEFINITIONS`. It initially reused Kasa-obake sprites as placeholders, then received a full 1024×1536 five-frame sprite set, a 1024×1024 portrait, and a small icon (`enemy_hitotsume_kozo_icon.png`). Column 8 battles are Kasa-obake only.
- **Hitotsume-kozō debuff behavior**: Added a new `debuff` ability type so `Frightening Gaze` and `Evil Eye` are prioritized like buffs (one per turn) and actually apply their `weak`/`slow`/`void_touched` effects. `Eye Bonk` weight lowered so the gaze abilities are used first.
- **Bake-neko enemy**: Added to Japanese Fields columns 4–8 with placeholder Kasa-obake sprites. Abilities: `Feline Agility` (evasion buff with neutralize/empower challenge), `Razor Claws` (physical + bleed), `Shadow Pounce` (void damage with multiplier challenge), and `Ghost Flame` (fire + burn). Added an evasion stat to `Character`, integrated it into player and enemy miss checks, and added `setChance`/`setValue` challenge outcomes for precise challenge rewards.
- Service worker cache bumped to `medoru-v226`; game bundle bumped to `game.js?v=326`.
- **Bake-neko defend sprite / battle polish**: Replaced `enemy_bake_neko_defend.png`, added a non-lethal defend-pose flash, stopped status icons on defeated enemies, fixed the missing "Use Item" action, made fresh profiles start with only shield socket charms, and reworked Sharpen Blade with a JSON-driven, stat-scaling turn duration system for buffs/debuffs.
- **Currency rework**: Renamed meta currencies to `ouroScales` (was `gameTokens`) and `ouroSource` (was `rareGameTokens`). Added `ouroEssence`, earned persistently: 1 per 7 normal enemies, 3 per 5 mini-bosses, 2 per level-1 map boss, 5% chance from chests, plus future events. Home camp shop and victory UI updated to show all three currencies.
- **Anonymous classroom tests**: Published tests in the featured public classroom (`/classrooms/:id/tests/:test_id`) can now be taken by anonymous users. A `TestSession` with `user_id: nil` tracks answers, `time_up`/`sync_time` are no-ops, and completion redirects to `/classrooms/:id/tests/:test_id/results?session_id=...`. Authenticated classroom members keep the existing attempt-based flow.

### What's Complete (v0.8.0) — Admin, Grammar, and Link Previews
- **Public Tests tab**: Added a "Tests" link to the anonymous site navigation (between "Lessons" and "Games") and a new `/tests` page (`MedoruWeb.TestLive.Index`) that lists published tests from the featured classroom. Anonymous users can browse and start tests, which route to the existing public classroom test flow.
- **Authenticated classroom test scoring/time fix**: `ClassroomLive.Test` now completes the underlying `TestSession` when an authenticated student finishes a test, so the results page reads the correct score and max points instead of showing 0/0. Timer sync events also update the server's `time_remaining` assign, and completion falls back to wall-clock elapsed time when the timer hasn't decremented yet, preventing `0:00` time spent.
- **Custom lesson word cap raised to 70**: Vocabulary lessons in classrooms now allow up to 70 words (was 60). The limit is enforced in the `CustomLesson` schema, the teacher lesson editor UI, `Content.add_word_to_lesson/3`, and the image-vocabulary lesson builder.
- **Admin kanji reading reordering**: Added a `position` column to `kanji_readings` and backfilled existing rows. The admin kanji edit page (`/admin/kanji/:id/edit`) now lists readings in position order and supports drag-and-drop reordering via the `ReadingSorter` JS hook. New readings are automatically assigned the next position, and existing reading creation, deletion, and AI-enrichment flows are unchanged.
- **Learned kanji search, filter, and sort**: The `/users/:id/kanji` page now has a search bar (character/meaning/reading), JLPT N1–N5 and school-grade SL1–SL6 filter buttons, and a sort dropdown (recently learned, oldest learned, character, JLPT level, stroke count, known score, frequency). Filtering and sorting are server-side via `Learning.list_learned_kanji_filtered/2`, pagination works with active filters, and the controls are responsive and fully gettext-wrapped.
- **Game title screen**: Added `TitleScene` as the entry point after `BootScene`. Shows the game title, and dynamic buttons for **Continue** (only when a saved run exists), **New Run**, and **Settings**. Choosing New Run while a run is active shows a confirmation dialog and, if confirmed, clears the active run via `Player.resetToFreshHero()` and starts hero selection.
...
- **Learned kanji practice pagination**: The `/users/:id/kanji/practice` page paginates learned kanji at 30 per page (matching `/users/:id/kanji`). Selection state is preserved across pages, so users can choose kanji from any page for drawing practice.
...
- **Grammar lesson pseudo-HTML markdown crash fix**: Opening a grammar lesson (`/classrooms/:id/custom-lessons/:lesson_id`) no longer crashes with a `MatchError` when the step explanation contains pseudo-HTML tags like `<English>...</English>`. All `render_markdown/1` helpers now handle `Earmark.as_html/2` returning `{:error, html, warnings}` by rendering the partial HTML instead of raising.
- **Custom lesson publish state tied to classroom publications**: A custom lesson now transitions to `status: "published"` only when it is published to at least one classroom (`Content.publish_lesson_to_classroom/4`). When a lesson is unpublished from its last classroom (`Content.unpublish_lesson_from_classroom/2`), its status reverts to `"draft"`. `publish_custom_lesson/1` now enforces the same rule and returns `{:error, :not_published_to_classroom}` if there are no active classroom publications. The teacher "Publish" button in the lesson editor and grammar form now redirects to the classroom publish page without changing status until a classroom is chosen.
...
1. **Shield Socket 1 families** (no abilities yet):
   ...
   - `warding` — elemental guard ability (can use existing elemental guard buff effects).
...
- A new entry in `assets/js/game/data/abilities/warrior.json` with `requiredCharmFamily` set.
- The family → ability mapping in `assets/js/game/data/abilityFamilies.json`.

Optional follow-ups after the above:
- Add more Slot 2 passive charms for weapons and shields.
- Add new status effects to `EffectRegistry.js` only if a family needs a mechanic that does not exist yet (e.g. a generic defense-up buff).
...
### What's Complete (v0.5.1) — The Hollow Ouroboros Battle MVP (v1.0.0-prealpha)
