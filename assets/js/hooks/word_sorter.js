 ...
 * WordSorter Hook
 *
 * Provides drag-and-drop reordering functionality for lesson words.
...
const WordSorter = {
  ...
  initSortable() {
    const items = this.container.querySelectorAll('[data-word-id]');
    ...
    items.forEach(item => {
      ...
      item.addEventListener('dragstart', (e) => {
        ...
        e.dataTransfer.setData('text/plain', item.dataset.wordId);
      });
  ...
  getDragAfterElement(container, y) {
    const draggableElements = [...container.querySelectorAll('[data-word-id]:not(.dragging)')];
  ...
  updateOrder() {
    const items = this.container.querySelectorAll('[data-word-id]');
    const wordIds = Array.from(items).map(item => item.dataset.wordId);

    // Push event to LiveView
    this.pushEvent('reorder', { word_ids: wordIds });
  },
