defmodule MedoruWeb.Admin.WordLive.Index do
  @moduledoc """
  ...
  def handle_event("filter_difficulty", %{"difficulty" => difficulty}, socket) do
    ...
    {:noreply,
     socket
     |> push_patch(
       to: ~p"/admin/words?#{%{difficulty: diff_param, search: socket.assigns.search}}"
     )}
  end
  ...
  def handle_event("search", %{"search" => %{"query" => query}}, socket) do
    ...
    {:noreply,
     ...
     |> push_patch(
       to:
         ~p"/admin/words?#{%{difficulty: socket.assigns.difficulty_filter, search: search_param}}"
     )}
  end

  @impl true
  def handle_event("clear_filters", _, socket) do
    {:noreply, socket |> push_patch(to: ~p"/admin/words")}
  end
