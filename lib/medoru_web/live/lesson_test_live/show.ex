defmodule MedoruWeb.LessonTestLive.Show do
  ...
  def handle_event("handle_key", %{"key" => key}, socket) do
    ...
    cond do
      ...
      step.question_type == :reading_text && socket.assigns.feedback == :incorrect &&
          key in ["Enter", "ArrowRight"] ->
        handle_event("continue_after_correction", %{}, socket)
        ...
        <%= if @current_step do %>
          <div class="bg-base-100 rounded-2xl shadow-sm border border-base-200 p-6 mb-6">
            ...
            <%= if @show_stroke_preview && @preview_kanji do %>
              <MedoruWeb.LessonTestLive.WritingComponent.stroke_preview
                kanji={@preview_kanji}
                locale={@locale}
              />
            <% else %>
              ...
              <div class="mb-6">
                ...
                <% else %>
                  <% display = step_display_info(@current_step) %>
                  <%= if display.label do %>
  ...
  # with existing tests that don't have type in question_data
  ...
  defp step_display_info(step) do
    ...
    case type do
      ...
      _ ->
        %{
          label: translate_question(step.question),
          main_text: nil,
          main_class: "",
          sub_text: nil,
          sub_class: ""
