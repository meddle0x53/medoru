defmodule MedoruWeb.Moderator.GrammarDefinitionLive.Index do
  @moduledoc """
  Moderator interface for listing and managing grammar definitions.
  """
  ...
  def handle_params(params, _url, socket) do
    ...
    {:noreply,
     socket
     |> assign(:page_title, gettext("Moderator - Grammar"))
  ...
  def handle_event("filter_level", %{"level" => level}, socket) do
    ...
    {:noreply,
     socket
     |> push_patch(
       to: ~p"/moderator/grammars?#{%{level: level_param, search: socket.assigns.search}}"
     )}
  end
  ...
  def handle_event("search", %{"search" => %{"query" => query}}, socket) do
    ...
    {:noreply,
     socket
     |> push_patch(
       to: ~p"/moderator/grammars?#{%{level: socket.assigns.level_filter, search: search_param}}"
     )}
  end

  @impl true
  def handle_event("clear_filters", _, socket) do
    {:noreply, socket |> push_patch(to: ~p"/moderator/grammars")}
  end
    ...
    case Content.delete_grammar_definition(grammar_definition) do
      {:ok, _} ->
        {:noreply,
         ...
         |> push_patch(to: ~p"/moderator/grammars")}
