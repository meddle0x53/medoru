defmodule MedoruWeb.Teacher.GameLive.Show do
  @moduledoc """
  LiveView for teachers to view a game's details and rankings.
  """
          ...
          <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div>
              ...
              <p class="text-secondary">
                {gettext("Memory Card Game")} · {@game.memory_card_game.board_size}
              </p>
            </div>
            <div class="flex gap-2">
              ...
              <% end %>
              <.link
                navigate={~p"/teacher/classrooms/#{@classroom.id}/games/#{@game.id}/edit"}
          ...
          <div class="card-body">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <div class="bg-base-200 p-4 rounded-lg">
                ...
                <p class="text-lg font-bold text-base-content">{@game.memory_card_game.board_size}</p>
              </div>
              <div class="bg-base-200 p-4 rounded-lg">
                <p class="text-secondary text-sm">{gettext("Max Attempts")}</p>
                <p class="text-lg font-bold text-base-content">{@game.memory_card_game.max_attempts}</p>
              </div>
              <div class="bg-base-200 p-4 rounded-lg">
              ...
              <div class="bg-base-200 p-4 rounded-lg">
                ...
                <p class="text-lg font-bold text-base-content">
                  {collection_label(@game.memory_card_game)}
                </p>
              </div>
            </div>
          </div>
        ...
        <div class="card bg-base-100 border border-base-300 shadow-sm mb-6">
          <div class="card-body">
            <h2 class="card-title text-base-content mb-4">
              {gettext("Words")}
              <span class="badge badge-ghost ml-2">{length(@game.memory_card_game.memory_card_game_words)}</span>
            </h2>
            <div class="flex flex-wrap gap-2">
              <%= for mgw <- @game.memory_card_game.memory_card_game_words |> Enum.sort_by(& &1.position) do %>
                <div class="badge badge-outline badge-lg gap-1">
                  <span class="font-medium">{mgw.word.text}</span>
                  <span class="text-secondary text-xs">{mgw.points}pts</span>
                </div>
              <% end %>
            </div>
          </div>
  ...
  defp collection_label(mcg) do
    type = Medoru.Games.MemoryCardGame.collection_type(mcg)

    case type do
      :direct -> gettext("Direct")
      :meaning -> gettext("Meaning")
      :pronunciation -> gettext("Pronunciation")
      :meaning_or_pronunciation -> gettext("Meaning OR Pronunciation")
      :meaning_and_pronunciation -> gettext("Meaning AND Pronunciation")
      _ -> gettext("Unknown")
    end
  end
end
