defmodule MedoruWeb.Teacher.TestLive.Edit do
  ...
  def mount(%{"id" => id}, session, socket) do
    ...
    if not Tests.is_test_owner?(test, user.id) do
      ...
      if test.setup_state != "in_progress" do
        ...
        # Load grammar-related data for grammar step types
        ...
        socket =
          ...
          |> assign(:extracted_writing_fill_in_steps, nil)
          |> assign(:writing_fill_in_examples, nil)
          |> assign(:writing_fill_in_image_loading, false)
  ...
  def handle_event("select_step_type", %{"type" => type_str}, socket) do
    ...
    {type, step_type} =
      case type_str do
        ...
        "writing_fill_in" -> {:writing_fill_in, "vocabulary"}
  ...
  @impl true
  def handle_event("validate_writing_fill_in_image", _params, socket) do
    {:noreply, socket}
  end

  @impl true
  def handle_event("extract_writing_fill_in_image", _params, socket) do
    case consume_uploaded_entries(socket, :test_step_image, fn %{path: path}, _entry ->
           {:ok, File.read!(path)}
         end) do
      [] ->
      ...
      [image_binary] ->
        {:noreply,
         socket
         |> assign(:writing_fill_in_image_loading, true)
         |> assign(:error, nil)
         |> start_async(:extract_writing_fill_in, fn ->
           Medoru.AI.ImageTestSteps.extract_writing_fill_in_steps(image_binary)
         end)}
    end
  end
  ...
  @impl true
  def handle_event(
        "update_extracted_writing_fill_in_step",
        %{"index" => index, "field" => field, "value" => value},
        socket
      ) do
    index = String.to_integer(index)
    steps = socket.assigns.extracted_writing_fill_in_steps || []
    ...
    {:noreply, assign(socket, :extracted_writing_fill_in_steps, updated_steps)}
  end

  @impl true
  def handle_event("save_writing_fill_in_steps", _params, socket) do
    test = socket.assigns.test
    examples = socket.assigns.writing_fill_in_examples || []
    steps = socket.assigns.extracted_writing_fill_in_steps || []
    ...
    if steps == [] do
      ...
      results =
        ...
        |> Enum.map(fn {step, i} ->
          ...
          attrs = %{
            ...
            "question" => gettext("Fill in the blanks"),
            "points" => 10,
            "correct_answer" => step["correct_answer"] || "",
            "question_data" => %{
              "examples" => examples,
      ...
      if errors != [] do
        ...
        Enum.each(errors, fn {:error, changeset} ->
          Logger.error("Failed to save writing fill-in step: #{inspect(changeset.errors)}")
        end)
      end
  ...
  def handle_event("add_pattern_element", %{"type" => type}, socket) do
    current_elements = socket.assigns.grammar_pattern_elements || []
    ...
    elements = current_elements ++ [new_element]

    # Update question_data in changeset
    changeset = socket.assigns.step_changeset

    question_data =
      (Changeset.get_field(changeset, :question_data) || %{})
      |> Map.put("pattern", elements)

    changeset =
      changeset
      |> Changeset.put_change(:question_data, question_data)
  ...
  @impl true
  # Handle phx-click events from buttons/checkboxes
  def handle_event(
        "update_pattern_element",
        %{"index" => index, "field" => field, "value" => value},
        socket
      ) do
    update_pattern_element_by_index(socket, String.to_integer(index), field, value)
  end

  @impl true
  # Handle phx-change events from form selects/inputs
  def handle_event(
        "update_pattern_element",
        %{"_target" => target, "step" => step_params},
        socket
      ) do
    # Extract index and field from _target path like ["step", "question_data", "pattern", "0", "grammar_form"]
    case target do
      ["step", "question_data", "pattern", index_str, field] ->
        index = String.to_integer(index_str)
        # Get the value from nested params
        value =
          get_in(step_params, ["question_data", "pattern", index_str, field])

        update_pattern_element_by_index(socket, index, field, value)
  ...
  @impl true
  def handle_async(:extract_writing_fill_in, {:ok, {:ok, data}}, socket) do
    steps = data["steps"] || []
    examples = data["examples"] || []
    ...
    if steps == [] do
      {:noreply,
       ...
       |> put_flash(
         :error,
         gettext("No writing fill-in steps could be extracted from the image.")
       )}
    else
      {:noreply,
       socket
       |> assign(:extracted_writing_fill_in_steps, steps)
       |> assign(:writing_fill_in_examples, examples)
       |> assign(:writing_fill_in_image_loading, false)}
    end
  end

  @impl true
  def handle_async(:extract_writing_fill_in, {:ok, {:error, msg}}, socket) do
    {:noreply,
     socket
     |> assign(:writing_fill_in_image_loading, false)
     |> assign(:error, msg)
  ...
  @impl true
  def handle_async(:extract_writing_fill_in, {:exit, reason}, socket) do
    {:noreply,
     socket
     |> assign(:writing_fill_in_image_loading, false)
     |> assign(:error, inspect(reason))
        ...
        <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
          <div class="bg-base-100 rounded-2xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div class="p-6 space-y-6">
              ...
              <%= if @step_type == :writing_fill_in && is_nil(@editing_step) && is_nil(@extracted_writing_fill_in_steps) do %>
                <div>
                  <label class="block text-sm font-medium text-base-content mb-2">
                    {gettext("Extract from Image")}
                  </label>
                  <form
                    id="writing-fill-in-image-form"
                    phx-change="validate_writing_fill_in_image"
                    phx-submit="extract_writing_fill_in_image"
                    class="space-y-3"
                  >
                    <div
                      class="border-2 border-dashed border-base-300 rounded-xl p-6 text-center hover:border-primary/50 transition-colors cursor-pointer"
              ...
              <%= if (@step_type != :grammar_pattern || is_nil(@extracted_grammar_pattern_steps)) && (@step_type != :writing_fill_in || is_nil(@extracted_writing_fill_in_steps)) do %>
                <.form
                  for={@step_form}
                  as={:step}
                  id="step-form"
                ...
                >
                  ...
                  <% else %>
                    ...
                    <% else %>
                      ...
                      <%= if @step_type == :writing_fill_in do %>
                        <WritingFillInStepForm.writing_fill_in_form
                          step_form={@step_form}
                          step_changeset={@step_changeset}
                        />
                      <% else %>
              ...
              <%= if @step_type == :writing_fill_in && @extracted_writing_fill_in_steps do %>
                <div class="space-y-6">
                  <%!-- Examples --%>
                  <%= if @writing_fill_in_examples && @writing_fill_in_examples != [] do %>
                    <div class="bg-base-200 rounded-lg p-4 space-y-2">
                      <div class="text-sm text-secondary mb-1">{gettext("Examples")}</div>
                      <%= for example <- @writing_fill_in_examples do %>
                        <div class="font-jp text-base-content">{example}</div>
                      <% end %>
                    </div>
                  <% end %>
                  ...
                  <div class="space-y-4">
                    <h4 class="font-medium">
                      {gettext("Extracted Steps")}
                      <span class="badge badge-primary badge-sm ml-2">
                        {length(@extracted_writing_fill_in_steps)}
                      </span>
                    </h4>

                    <%= for {step, index} <- Enum.with_index(@extracted_writing_fill_in_steps) do %>
                      <div class="bg-base-100 border border-base-200 rounded-lg p-4 space-y-3">
                        <div class="flex items-center gap-2">
                          <span class="text-sm text-secondary font-medium">#{step["number"]}</span>
                        </div>
                        ...
                        <div>
                          ...
                          <textarea
                            phx-change="update_extracted_writing_fill_in_step"
                            phx-value-index={index}
                            phx-value-field="template"
                            phx-debounce="300"
                            class="textarea textarea-bordered textarea-sm w-full font-jp"
                            rows="2"
                          ><%= step["template"] || "" %></textarea>
                        </div>
                  ...
                  <div class="flex justify-end gap-3">
                    ...
                    <button
                      type="button"
                      phx-click="save_writing_fill_in_steps"
                      class="btn btn-primary"
                    >
                      <.icon name="hero-plus" class="w-5 h-5 mr-2" />
                      {gettext("Save All Steps")}
  ...
  defp format_question_type(:writing_fill_in), do: gettext("Writing Fill In")
  ...
  defp parse_options_from_params(%{"question_data" => question_data} = params)
    ...
    # Normalize pattern from map (form data) to list if present
    question_data = normalize_pattern_in_question_data(question_data)

    # Continue processing with normalized question_data
    params
    |> Map.put("question_data", question_data)
    |> parse_options_from_params()
  end
  ...
  # Normalize pattern from map with string keys ("0", "1", etc.) to a list
  defp normalize_pattern_in_question_data(%{"pattern" => pattern} = question_data)
       when is_map(pattern) do
    # Convert map like %{"0" => elem0, "1" => elem1} to list [elem0, elem1]
    normalized_pattern =
      pattern
      |> Enum.sort_by(fn {k, _v} -> String.to_integer(k) end)
      |> Enum.map(fn {_k, v} -> v end)
      |> Enum.map(&add_type_to_element/1)

    Map.put(question_data, "pattern", normalized_pattern)
  end
  ...
  defp add_type_to_element(%{"word_type" => _} = element),
    do: Map.put(element, "type", "word_slot")

  defp add_type_to_element(%{"word_class_id" => _} = element),
    do: Map.put(element, "type", "word_class")

  defp add_type_to_element(%{"text" => _} = element), do: Map.put(element, "type", "literal")
  defp add_type_to_element(element), do: Map.put(element, "type", "literal")
  ...
  defp update_pattern_element_by_index(socket, index, field, value) do
    ...
    elements =
      List.update_at(elements, index, fn element ->
        case field do
          "word_type" ->
            # Reset grammar_form when word_type changes
            element
            |> Map.put("word_type", value)
            |> Map.put("grammar_form", nil)

          "word_class_id" ->
            # Find the word class name
            word_class =
              Enum.find(socket.assigns.word_classes, fn wc ->
                to_string(wc.id) == value
              end)

            element
            |> Map.put("word_class_id", value)
            |> Map.put("word_class_name", if(word_class, do: word_class.display_name, else: nil))

          "text" ->
            Map.put(element, "text", value)

          "optional" ->
            # Handle checkbox value - can be string or boolean
            is_optional =
