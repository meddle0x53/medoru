defmodule Mix.Tasks.Medoru.FixSplitLessons do
  ...
  defp remove_duplicates do
    ...
    total_removed =
      Enum.reduce(duplicates, 0, fn {title, _count}, acc ->
  ...
  defp delete_lessons(lessons_to_delete) do
    ...
    test_ids =
      Enum.filter(lessons_to_delete, fn {_, test_id} -> test_id end)
      |> Enum.map(fn {_, test_id} -> test_id end)
  ...
  defp rename_to_one_based do
    ...
    # Group by base name
    grouped =
      Enum.group_by(all_numbered, fn lesson ->
    ...
    Enum.each(grouped, fn {_base_name, lessons} ->
      ...
      |> Enum.each(fn {lesson, new_index} ->
        old_number =
          case Regex.run(~r/\\((\\d+)\\)$/, lesson.title) do
            [_, num] -> num
            _ -> "0"
          end
