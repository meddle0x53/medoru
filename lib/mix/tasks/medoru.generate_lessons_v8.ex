defmodule Mix.Tasks.Medoru.GenerateLessonsV8 do
  ...
  defp categorize_word(word) do
    ...
    # Greetings
    topics = if matches?(combined, ["hello", "goodbye", "morning", "evening", "night", "greeting", "hi ", "bye"]),
      do: [:greetings | topics], else: topics

    # Self intro
    topics = if matches?(combined, ["name", "call", "self", "introduction", "nice to meet", "pleased"]),
      do: [:self_intro | topics], else: topics

    # Pronouns
    topics = if matches?(combined, ["watashi", "boku", "anata", "kare", "kanojo", "i ", "you", "he ", "she "]),
      do: [:pronouns | topics], else: topics

    # Family
    topics = if matches?(combined, ["father", "mother", "parent", "brother", "sister", "family", "child", "son", "daughter"]),
      do: [:family | topics], else: topics

    # People/Occupations
    topics = if matches?(combined, ["person", "people", "teacher", "student", "doctor", "worker", "company"]),
      do: [:people, :occupations | topics], else: topics

    # Days/Time
    topics = if matches?(combined, ["monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday", "week"]),
      do: [:days | topics], else: topics

    topics = if matches?(combined, ["hour", "minute", "time", "o'clock", "half", "am", "pm", "morning", "evening", "noon"]),
      do: [:time | topics], else: topics

    topics = if matches?(combined, ["january", "february", "march", "month", "year", "date", "calendar", "birthday"]),
      do: [:calendar | topics], else: topics

    topics = if matches?(combined, ["today", "tomorrow", "yesterday", "day before", "day after", "last week", "next week"]),
      do: [:time_exp | topics], else: topics

    # Verbs
    topics = if matches?(combined, ["desu", "is ", "am ", "are "]) and word.word_type == "expression",
      do: [:be_verbs | topics], else: topics

    topics = if matches?(combined, ["go", "come", "return", "arrive", "leave"]) and word.word_type == "verb",
      do: [:move_verbs | topics], else: topics

    topics = if matches?(combined, ["eat", "drink", "tabemasu", "nomimasu"]) and word.word_type == "verb",
      do: [:eat_verbs | topics], else: topics

    topics = if matches?(combined, ["see", "look", "watch", "read", "write", "speak", "say", "hear"]) and word.word_type == "verb",
      do: [:action_verbs | topics], else: topics

    topics = if matches?(combined, ["do", "make", "take", "give", "receive", "use"]) and word.word_type == "verb",
      do: [:daily_verbs | topics], else: topics

    # Food
    topics = if matches?(combined, ["rice", "bread", "noodle", "meat", "fish", "vegetable", "fruit", "food"]),
      do: [:food1, :food2 | topics], else: topics

    topics = if matches?(combined, ["sushi", "tempura", "curry", "soup", "salad", "dish", "meal"]),
      do: [:food2 | topics], else: topics

    topics = if matches?(combined, ["water", "tea", "coffee", "juice", "milk", "drink", "beer", "sake", "alcohol"]),
      do: [:drinks | topics], else: topics

    # Places
    topics = if matches?(combined, ["house", "home", "room", "kitchen", "bedroom", "bathroom", "door", "window"]),
      do: [:home | topics], else: topics

    topics = if matches?(combined, ["school", "teacher", "student", "class", "lesson", "study", "learn", "book"]),
      do: [:school | topics], else: topics

    topics = if matches?(combined, ["train", "bus", "car", "bicycle", "station", "airport", "transport"]),
      do: [:transport | topics], else: topics

    topics = if matches?(combined, ["store", "shop", "restaurant", "hospital", "bank", "park", "building", "place"]),
      do: [:places | topics], else: topics

    # Adjectives
    topics = if matches?(combined, ["big", "small", "long", "short", "tall", "round", "square", "size", "shape"]) and word.word_type == "adjective",
      do: [:size_adj | topics], else: topics

    topics = if matches?(combined, ["good", "bad", "new", "old", "young"]) and word.word_type == "adjective",
      do: [:quality_adj | topics], else: topics

    topics = if matches?(combined, ["happy", "sad", "fun", "interesting", "tasty", "delicious", "boring"]) and word.word_type == "adjective",
      do: [:feeling_adj | topics], else: topics

    topics = if matches?(combined, ["red", "blue", "white", "black", "green", "yellow", "color", "colour"]),
      do: [:colors | topics], else: topics

    # Shopping/Money
    topics = if matches?(combined, ["buy", "sell", "shop", "shopping", "store", "pay"]),
      do: [:shopping | topics], else: topics

    topics = if matches?(combined, ["money", "yen", "dollar", "cheap", "expensive", "price", "cost", "free"]),
      do: [:money | topics], else: topics

    # Nature
    topics = if matches?(combined, ["weather", "rain", "snow", "sun", "cloud", "wind", "hot", "cold", "warm", "cool"]),
      do: [:weather | topics], else: topics

    topics = if matches?(combined, ["mountain", "river", "sea", "ocean", "tree", "flower", "forest", "nature"]),
      do: [:nature | topics], else: topics

    topics = if matches?(combined, ["dog", "cat", "bird", "fish", "animal", "pet"]),
      do: [:animals | topics], else: topics

    # Body/Health
    topics = if matches?(combined, ["head", "face", "eye", "ear", "nose", "mouth", "hand", "foot", "body", "leg", "arm"]),
      do: [:body | topics], else: topics

    topics = if matches?(combined, ["health", "sick", "ill", "medicine", "doctor", "hospital", "pain", "hurt"]),
      do: [:health | topics], else: topics

    # Clothing
    topics = if matches?(combined, ["shirt", "pants", "dress", "skirt", "shoe", "hat", "clothes", "wear", "sock"]),
      do: [:clothing | topics], else: topics

    # Items
    topics = if matches?(combined, ["phone", "computer", "bag", "key", "wallet", "watch", "clock", "umbrella"]),
      do: [:items | topics], else: topics

    # Directions
    topics = if matches?(combined, ["right", "left", "straight", "turn", "direction", "north", "south", "east", "west"]),
      do: [:directions | topics], else: topics

    # Hobbies
    topics = if matches?(combined, ["hobby", "sport", "music", "movie", "game", "play", "read", "dance", "sing"]),
      do: [:hobbies | topics], else: topics

    # Travel
    topics = if matches?(combined, ["travel", "trip", "hotel", "passport", "luggage", "sightseeing", "foreign", "abroad"]),
      do: [:travel | topics], else: topics

    # Emotions
    topics = if matches?(combined, ["like", "love", "hate", "want", "need", "think", "feel", "emotion"]),
      do: [:emotions | topics], else: topics
  ...
  defp generate_lessons(words_by_topic, dry_run) do
    ...
    Enum.reduce(@lesson_definitions, {[], used_ids}, fn def, {acc, used} ->
      if dry_run do
        ...
        Logger.info("  Topic: #{def.topic}, Available: #{length(available)}, New: #{length(selected)}")
  ...
  defp create_lesson(def, words_by_topic, used_ids) do
    ...
    if length(selected) == 0 do
      ...
      attrs = %{
        title: title,
        description: "N5 Lesson #{def.order}: #{def.title}. Learn #{length(selected)} new words about #{def.topic}.",
        difficulty: 5,
        order_index: def.order * 100,
        lesson_type: :reading
      }

      links = Enum.with_index(selected, fn word, idx ->
  ...
  defp delete_existing_n5_lessons do
    ...
    Ecto.Adapters.SQL.query!(Repo, "DELETE FROM lesson_words WHERE lesson_id IN (SELECT id FROM lessons WHERE difficulty = 5)")
    Ecto.Adapters.SQL.query!(Repo, "DELETE FROM lesson_progress WHERE lesson_id IN (SELECT id FROM lessons WHERE difficulty = 5)")
  ...
  defp show_samples do
    ...
    |> Repo.all()
    |> Enum.each(fn lesson ->
      count = Repo.aggregate(from(lw in LessonWord, where: lw.lesson_id == ^lesson.id), :count, :id)
