defmodule Mix.Tasks.Medoru.GenerateLessonsV9 do
  ...
  defp create_lesson(lesson_def, difficulty) do
    ...
    if length(word_ids) == 0 do
      ...
      attrs = %{
        title: title,
        description: "#{level} Lesson #{lesson_def.order}: #{lesson_def.title}. Learn #{length(word_ids)} essential words.",
        difficulty: difficulty,
        order_index: lesson_def.order * 100,
        lesson_type: :reading
      }
      
      links = Enum.with_index(word_ids, fn word_id, idx ->
  ...
  defp delete_existing_lessons(difficulty) do
    ...
    Ecto.Adapters.SQL.query!(Repo, "DELETE FROM lesson_words WHERE lesson_id IN (SELECT id FROM lessons WHERE difficulty = #{difficulty})")
