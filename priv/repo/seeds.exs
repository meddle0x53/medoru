...
if File.exists?(kanji_path) do
  ...
  Enum.each(kanji_list, fn k ->
    readings = Enum.map(k["readings"] || [], fn r ->
      %{
  ...
  valid_words = 
    ...
    |> Enum.filter(fn w ->
      ...
      String.length(text) <= 255 and
        String.match?(text, ~r/[\x{4e00}-\x{9fff}\x{3040}-\x{309f}\x{30a0}-\x{30ff}\x{ff00}-\x{ffef}ー]+/u)
    end)
    |> Enum.map(fn w ->
      ...
      meaning = w["meaning"]
      truncated_meaning = if String.length(meaning) > 250, do: String.slice(meaning, 0, 250) <> "...", else: meaning
